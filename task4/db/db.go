...
func (db *DB) AddNewPost(title string, content string) {
	_, err := db.sql.Exec("INSERT INTO "+databaseName+"."+tableName+" (title,content) VALUES (?,?);", title, content)
	if err != nil {
		log.Println(err)
	}
}
...
func (db *DB) IsPostExists(id int) bool {
	var isExists bool
	err := db.sql.QueryRow("SELECT EXISTS(SELECT id FROM "+tableName+" WHERE id = ?)", id).Scan(&isExists)
	if err != nil {
		log.Println(err)
	}
	return isExists
...
func (db *DB) UpdatePost(id int, title string, content string) {
	_, err := db.sql.Exec("UPDATE "+tableName+" SET title = ?, content = ? WHERE id = ?", title, content, id)
	if err != nil {
		log.Println(err)
	}
}
...
func (db *DB) GetCountPosts() int {
	var count int
	err := db.sql.QueryRow("SELECT count(*) as count FROM " + tableName).Scan(&count)
	if err != nil {
		log.Println(err)
	}
	return count
...
func (db *DB) GetAllPosts() map[int]models.Post {
	...
	rows, err := db.sql.Query("SELECT id, title, content FROM " + tableName)
	if err != nil {
		log.Println(err)
	}
	defer rows.Close()
...
func (db *DB) GetPostByID(id int) models.Post {
	var title string
	var content string
	err := db.sql.QueryRow("SELECT title, content FROM "+tableName+" WHERE id = ?", id).Scan(&title, &content)
	if err != nil {
		log.Println(err)
	}
