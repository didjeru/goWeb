...
func main() {
	...
	router.HandleFunc("/new", server.handleNewPost)
...
func (server *Server) handleNewPost(wr http.ResponseWriter, req *http.Request) {
	tmpl := getTemplate(server.Templates, Edit)
	if tmpl == nil {
		err := errors.New("empty template")
		http.Error(wr, err.Error(), http.StatusInternalServerError)
	...
	if err := newPost(server.db, post); err != nil {
		log.Print(err)
		return
	}
