...
func editPost(db *sql.DB, post Post, id string) error {
	query := fmt.Sprintf(`UPDATE posts.habr_posts SET title="%s", date="%s", link="%s", comment="%s"
		where id=?;`, post.Title, post.Date, post.Link, post.Comment)
	_, err := db.Exec(query, id)

	return err
}

func newPost(db *sql.DB, post Post) error {

	query := fmt.Sprintf(`INSERT INTO posts.habr_posts (title, date, link, comment) 
		VALUES ("%s", "%s", "%s", "%s");`, post.Title, post.Date, post.Link, post.Comment)
